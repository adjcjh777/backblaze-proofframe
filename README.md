...
`scripts/judge_evidence_index.py` builds the public-safe evidence index behind `GET /api/judge/evidence-index`, collecting the judge brief, criteria crosswalk, final gate, Devpost preview, recording assets, and live-proof gates without changing `safe_to_submit=false`.
