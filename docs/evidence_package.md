...
| Public HF Space judge-mode sync | HF Space commit `b01aef5`; runtime sha `b01aef52a0c4be7a4c976fcfc199ec2015f7db59`; `python scripts/public_space_sync.py` passes with no failed checks; public API smoke passes in local/mock mode; raw `docs/assets/devpost-event-snapshot.json` shows 372 observed participants with submission open; raw `docs/assets/public-video-check.json` is part of the Space sync gate and exposes `video_url_official_public_host` for YouTube/Vimeo/Youku-only final video URLs | Public-ready in local/mock mode |
