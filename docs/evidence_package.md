...
| Public HF Space judge-mode sync | HF Space commit `a0418b6`; runtime sha `a0418b6da52376ca3984294c850e368552920404`; public HTML contains `Judge recording slate`, `Sponsor Evidence Model`, `shouldAutoLoadJudgeDemo`, and `Final reports pending`; `/api/submission/gate` returns `pre_live_safe` with `pre_live_packet_pending` and `report_gate`; raw Space `docs/assets/agent-handoff-report.json` returns `handoff_ready`; API smoke passes | Public-ready in local/mock mode |
