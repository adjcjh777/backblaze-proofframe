...
| Final live proof runner | `scripts/run_final_live_proof.py --env-file .env.final.local --preflight-only`, then `scripts/run_final_live_proof.py --env-file .env.final.local --evidence-out docs/assets/final-live-proof-evidence.json`; OpenAI fallback adds `--genblaze-provider openai --genblaze-image-model gpt-image-1` | Public-ready; starts and stops local app without committing logs or secrets |
