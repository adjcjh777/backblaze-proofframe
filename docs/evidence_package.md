...
| Public HF Space judge-mode sync | HF Space commit `aeba0ea`; runtime sha `aeba0eae4af5c9d8a50b46b29a2977d0cb961ccb`; `python scripts/public_space_sync.py` passes with no failed checks; public API smoke passes in local/mock mode; raw `docs/assets/devpost-event-snapshot.json` shows 372 observed participants with submission open; raw `docs/assets/final-submission-control.json` exposes `control_health_ok=true` while `safe_to_submit=false`; raw `docs/assets/judge-evidence-index.json` exposes `safe_to_submit=false` and public evidence links; raw `docs/assets/public-video-check.json` is part of the Space sync gate and exposes `video_url_official_public_host` for YouTube/Vimeo/Youku-only final video URLs | Public-ready in local/mock mode |
