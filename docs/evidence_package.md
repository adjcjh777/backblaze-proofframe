...
| Public HF Space judge-mode sync | HF Space commit `36ebc46`; runtime sha `36ebc46e15d51ef48118c0c3f6a647ca304f72f2`; public HTML contains `Judge recording slate`, `Sponsor Evidence Model`, `30-Second Judge Brief`, `Criteria Crosswalk`, `Recording Runbook`, `Devpost Kit`, `Submit Checklist`, `shouldAutoLoadJudgeDemo`, and `Final reports pending`; `/api/submission/gate` returns `pre_live_safe` with `pre_live_packet_pending` and `report_gate`; `/api/judge/crosswalk` returns `proofframe.judge_crosswalk.v1` with four rows and `safe_to_submit=false`; `/api/judge/recording` returns `proofframe.recording_assets.v1` with six shots and `final_video_ready=false`; `/api/judge/devpost` returns `proofframe.devpost_form_kit.v1` with 19 fields and `final_form_ready=false`; `/api/judge/submission-checklist` returns `proofframe.devpost_submission_checklist.v1` with preflight rows and `safe_to_submit=false`; raw Space `docs/assets/devpost-event-snapshot.json` returns a fresh dynamic participant count and `submission_open=true`; raw Space `docs/assets/final-submission-control.json` returns the same event snapshot context and `safe_to_submit=false`; raw Space `docs/assets/agent-handoff-report.json` returns `handoff_ready`; raw Space `docs/assets/final-launch-plan.json` returns `ready_for_credential_entry`; raw Space `docs/assets/b2-key-scope-checklist.json` returns `proofframe.b2_key_scope_checklist.v1` with `safe_to_commit=true`, pre-key confirmation status `required_before_key_creation`, bucket `proofframe-demo-a6b4e49`, prefix `campaigns/`, and required `writeFiles` plus `listAllBucketNames`; raw Space `docs/assets/judge-brief.json` returns `safe_to_submit=false`; raw Space `docs/assets/judge-crosswalk.json` returns `pre_live_crosswalk_ready` with `safe_to_submit=false`; raw Space `docs/assets/public-demo-screenshot-report.json` returns `public_judge_screenshot_ready` with visible/html markers verified and 741074 screenshot bytes; raw Space `docs/assets/recording-assets.json` returns `public_mock_verified` with `final_video_ready=false`; raw Space `docs/assets/demo-video-draft.json` returns `mock_video_draft_ready` with `safe_to_submit=false`; public MP4 `docs/assets/proofframe-demo-draft.mp4` returns `video/mp4` with 761356 bytes; raw Space `docs/assets/devpost-form-kit.json` returns `pre_live_form_ready` with `final_form_ready=false`; raw Space `docs/assets/devpost-submission-preview.json` returns `pre_live_preview_ready` with `safe_to_share=true` and `safe_to_submit=false`; raw Space `docs/assets/devpost-submission-checklist.json` returns `pre_submit_blocked` with `safe_to_submit=false`; API smoke and public_space_sync pass | Public-ready in local/mock mode |
...
| Public Space sync report | `scripts/public_space_sync.py`, `docs/assets/public-space-sync-report.json`, `docs/assets/public-space-sync-report.md` | Public-ready; verifies HF Space metadata/runtime sha, raw handoff report, raw Devpost event snapshot, raw final launch plan, raw B2 key scope checklist, raw judge brief, raw judge crosswalk, raw public screenshot report, raw recording assets, raw mock video draft report, public MP4 readability, raw Devpost kit, raw Devpost submission preview, raw submit checklist, raw post-credential live proof plan, raw submission bundle manifest, local/mock health, fail-closed submission gate, and judge HTML markers |
