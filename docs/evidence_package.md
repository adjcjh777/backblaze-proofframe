...
| Public HF Space judge-mode sync | HF Space commit `3308508`; public HTML contains `Judge recording slate`, `Sponsor Evidence Model`, and `shouldAutoLoadJudgeDemo`; `/api/submission/gate` returns `pre_live_safe` with current task gates; API smoke passes | Public-ready in local/mock mode |
