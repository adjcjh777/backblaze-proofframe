...
| Public HF Space judge-mode sync | HF Space commit `38d1263`; runtime sha `38d12631d87c785ac59a20e7d585b8f04bb840cc`; `python scripts/public_space_sync.py` passes with no failed checks; public API smoke passes in local/mock mode; raw `docs/assets/public-video-check.json` is now part of the Space sync gate and exposes `video_url_official_public_host` for YouTube/Vimeo/Youku-only final video URLs | Public-ready in local/mock mode |
