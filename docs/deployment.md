...
- Judge-mode slate, sponsor evidence model, in-app Judge Brief panel, in-app Criteria Crosswalk panel, in-app Recording Runbook panel, in-app Devpost Kit panel, in-app Submit Checklist panel, final report gate, Agent handoff report, final launch plan, judge brief, judge crosswalk, recording assets, mock video draft, Devpost form kit, submit checklist, and final control alignment verified after HF Space commit `bcd0bca`: runtime reports sha `bcd0bcad0bfcd912c38fd1f570a22112f61aead3`; public HTML contains `Judge recording slate`, `Sponsor Evidence Model`, `30-Second Judge Brief`, `Criteria Crosswalk`, `Recording Runbook`, `Devpost Kit`, `Submit Checklist`, `shouldAutoLoadJudgeDemo`, and `Final reports pending`; `/api/submission/gate` returns `pre_live_safe` with `pre_live_packet_pending` and `report_gate`; `/api/judge/crosswalk` returns `proofframe.judge_crosswalk.v1` with four rows and `safe_to_submit=false`; `/api/judge/recording` returns `proofframe.recording_assets.v1` with six shots and `final_video_ready=false`; `/api/judge/devpost` returns `proofframe.devpost_form_kit.v1` with 19 fields and `final_form_ready=false`; `/api/judge/submission-checklist` returns `proofframe.devpost_submission_checklist.v1` with preflight rows and `safe_to_submit=false`; raw Space artifacts `docs/assets/agent-handoff-report.json`, `docs/assets/final-launch-plan.json`, `docs/assets/judge-brief.json`, `docs/assets/judge-crosswalk.json`, `docs/assets/recording-assets.json`, `docs/assets/demo-video-draft.json`, `docs/assets/devpost-form-kit.json`, and `docs/assets/devpost-submission-checklist.json` return `handoff_ready`, `ready_for_credential_entry`, `safe_to_submit=false`, `pre_live_crosswalk_ready`, `public_mock_verified`, `mock_video_draft_ready`, `pre_live_form_ready`, and `pre_submit_blocked`; public MP4 `docs/assets/proofframe-demo-draft.mp4` returns `video/mp4` with 761356 bytes; public API smoke passes in local/mock mode.
