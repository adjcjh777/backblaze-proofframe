...
- Judge-mode slate, sponsor evidence model, in-app Judge Brief panel, Criteria crosswalk link, final report gate, Agent handoff report, final launch plan, judge brief, judge crosswalk, and final control alignment verified after HF Space commit `7700633`: runtime reports sha `77006332d53103cd6890f3b3cda446c096892554`; public HTML contains `Judge recording slate`, `Sponsor Evidence Model`, `30-Second Judge Brief`, `Criteria crosswalk`, `shouldAutoLoadJudgeDemo`, and `Final reports pending`; `/api/submission/gate` returns `pre_live_safe` with `pre_live_packet_pending` and `report_gate`; raw Space artifacts `docs/assets/agent-handoff-report.json`, `docs/assets/final-launch-plan.json`, `docs/assets/judge-brief.json`, and `docs/assets/judge-crosswalk.json` return `handoff_ready`, `ready_for_credential_entry`, `safe_to_submit=false`, and `pre_live_crosswalk_ready`; public API smoke passes in local/mock mode.
