...
- Judge-mode slate, sponsor evidence model, in-app Judge Brief panel, final report gate, Agent handoff report, final launch plan, judge brief, and final control alignment verified after HF Space commit `cbb686`: runtime reports sha `cbb686640283a157a6ab93a68ff5b0b5d7848977`; public HTML contains `Judge recording slate`, `Sponsor Evidence Model`, `30-Second Judge Brief`, `shouldAutoLoadJudgeDemo`, and `Final reports pending`; `/api/submission/gate` returns `pre_live_safe` with `pre_live_packet_pending` and `report_gate`; raw Space artifacts `docs/assets/agent-handoff-report.json`, `docs/assets/final-launch-plan.json`, and `docs/assets/judge-brief.json` return `handoff_ready`, `ready_for_credential_entry`, and `safe_to_submit=false`; public API smoke passes in local/mock mode.
