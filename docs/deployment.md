...
- Judge-mode slate, sponsor evidence model, in-app Judge Brief panel, in-app Criteria Crosswalk panel, final report gate, Agent handoff report, final launch plan, judge brief, judge crosswalk, and final control alignment verified after HF Space commit `413c5fe`: runtime reports sha `413c5fe90d97eb0a137a1fc4d5ca697e3b7cab14`; public HTML contains `Judge recording slate`, `Sponsor Evidence Model`, `30-Second Judge Brief`, `Criteria Crosswalk`, `shouldAutoLoadJudgeDemo`, and `Final reports pending`; `/api/submission/gate` returns `pre_live_safe` with `pre_live_packet_pending` and `report_gate`; `/api/judge/crosswalk` returns `proofframe.judge_crosswalk.v1` with four rows and `safe_to_submit=false`; raw Space artifacts `docs/assets/agent-handoff-report.json`, `docs/assets/final-launch-plan.json`, `docs/assets/judge-brief.json`, and `docs/assets/judge-crosswalk.json` return `handoff_ready`, `ready_for_credential_entry`, `safe_to_submit=false`, and `pre_live_crosswalk_ready`; public API smoke passes in local/mock mode.
