...
- Judge-mode slate, sponsor evidence model, in-app Judge Brief panel, in-app Criteria Crosswalk panel, in-app Recording Runbook panel, final report gate, Agent handoff report, final launch plan, judge brief, judge crosswalk, recording assets, and final control alignment verified after HF Space commit `4b9274b`: runtime reports sha `4b9274b78fee50c3a749ea7adadf768d61941507`; public HTML contains `Judge recording slate`, `Sponsor Evidence Model`, `30-Second Judge Brief`, `Criteria Crosswalk`, `Recording Runbook`, `shouldAutoLoadJudgeDemo`, and `Final reports pending`; `/api/submission/gate` returns `pre_live_safe` with `pre_live_packet_pending` and `report_gate`; `/api/judge/crosswalk` returns `proofframe.judge_crosswalk.v1` with four rows and `safe_to_submit=false`; `/api/judge/recording` returns `proofframe.recording_assets.v1` with six shots and `final_video_ready=false`; raw Space artifacts `docs/assets/agent-handoff-report.json`, `docs/assets/final-launch-plan.json`, `docs/assets/judge-brief.json`, `docs/assets/judge-crosswalk.json`, and `docs/assets/recording-assets.json` return `handoff_ready`, `ready_for_credential_entry`, `safe_to_submit=false`, `pre_live_crosswalk_ready`, and `public_mock_verified`; public API smoke passes in local/mock mode.
