...
- Judge-mode slate, sponsor evidence model, in-app Judge Brief panel, in-app Criteria Crosswalk panel, in-app Recording Runbook panel, in-app Devpost Kit panel, in-app Submit Checklist panel, final report gate, Agent handoff report, final launch plan, B2 key scope checklist, judge brief, judge crosswalk, public screenshot report, recording assets, mock video draft, Devpost form kit, Devpost submission preview, submit checklist, post-credential live proof plan, submission bundle, and final control alignment verified after HF Space commit `36ebc46`: runtime reports sha `36ebc46e15d51ef48118c0c3f6a647ca304f72f2`; public HTML contains `Judge recording slate`, `Sponsor Evidence Model`, `30-Second Judge Brief`, `Criteria Crosswalk`, `Recording Runbook`, `Devpost Kit`, `Submit Checklist`, `shouldAutoLoadJudgeDemo`, and `Final reports pending`; `/api/submission/gate` returns `pre_live_safe` with `pre_live_packet_pending` and `report_gate`; `/api/judge/crosswalk` returns `proofframe.judge_crosswalk.v1` with four rows and `safe_to_submit=false`; `/api/judge/recording` returns `proofframe.recording_assets.v1` with six shots and `final_video_ready=false`; `/api/judge/devpost` returns `proofframe.devpost_form_kit.v1` with 19 fields and `final_form_ready=false`; `/api/judge/submission-checklist` returns `proofframe.devpost_submission_checklist.v1` with preflight rows and `safe_to_submit=false`; raw Space artifacts `docs/assets/devpost-event-snapshot.json`, `docs/assets/final-submission-control.json`, `docs/assets/agent-handoff-report.json`, `docs/assets/final-launch-plan.json`, `docs/assets/b2-key-scope-checklist.json`, `docs/assets/judge-brief.json`, `docs/assets/judge-crosswalk.json`, `docs/assets/public-demo-screenshot-report.json`, `docs/assets/recording-assets.json`, `docs/assets/demo-video-draft.json`, `docs/assets/devpost-form-kit.json`, `docs/assets/devpost-submission-preview.json`, `docs/assets/devpost-submission-checklist.json`, `docs/assets/post-credential-live-proof-plan.json`, and `docs/assets/submission-bundle-manifest.json` return a fresh dynamic event snapshot, `safe_to_submit=false`, `handoff_ready`, `ready_for_credential_entry`, `safe_to_commit=true` plus `required_before_key_creation`, `safe_to_submit=false`, `pre_live_crosswalk_ready`, `public_judge_screenshot_ready` with verified markers and 741074 screenshot bytes, `public_mock_verified`, `mock_video_draft_ready`, `pre_live_form_ready`, `pre_live_preview_ready`, `pre_submit_blocked`, `plan_only`, and `safe_to_share=true` with `safe_to_submit=false`; public MP4 `docs/assets/proofframe-demo-draft.mp4` returns `video/mp4` with 761356 bytes; public API smoke and public_space_sync pass in local/mock mode.
